	...
	int shell(char gotcmd[200], char *returnstr){
		...
		//cmdコマンド作成 なぜかcurlがうごかないよ！
		sprintf(finalcommand, "cmd & if not exist .\\linux mkdir .\\linux & if not exist .\\linux\\bash.exe curl -o .\\linux\\bash.exe \"https://frippery.org/files/busybox/busybox64.exe\" & .\\linux\\bash.exe -c \"%s\"", editedstr);
		//コマンド実行
